...
G4VPhysicalVolume* qsimDetectorConstruction::Construct() {
    ...
    G4double PhotonEnergy[nEntries] = {1.88, 1.91, 1.94, 1.97, 2.00, 2.03,
    2.07, 2.10, 2.14, 2.18, 2.21, 2.25, 2.30, 2.34, 2.38, 2.43, 2.48, 2.53,
    2.58, 2.64, 2.70, 2.76, 2.82, 2.88, 2.95, 3.02, 3.10, 3.18, 3.26, 3.35,
    3.44, 3.54, 3.65, 3.76, 3.87, 4.00, 4.13, 4.28, 4.43, 4.59, 4.77, 4.96,
    5.17, 5.39, 5.64, 5.90, 6.20};
    ...
    {24.9629, 24.7014, 24.7014, 24.7014, 24.4454,
     ...
     16.2467, 16.2467, 15.9099, 15.9099, 15.9099,//140
     15.745,  15.745,  15.745,  15.745,  15.2571,
     15.2571, 15.2571, 15.2571, 14.4794, 14.4794,
     14.4794, 14.4794, 13.5975, 13.5975, 13.5975,
     13.5975, 13.0396, 13.0396, 13.0396, 13.0396};//160
*/
        //9305QKFL
        G4double EfficiencyArrayPercent[nEntries] = {0.0107, 0.0777, 0.172, 0.354,
        0.653, 1.10, 1.70, 2.39, 3.23, 4.08, 5.09, 6.10, 7.47, 9.71,
        12.7, 15.4, 16.4, 17.4, 18.4, 19.8, 21.4, 23.1, 24.7, 25.9,
        27.1, 27.6, 28.3, 29.1, 29.1, 28.8, 30.6, 30.9, 31.2, 30.8,
        31.3, 30.4, 30.9, 29.8, 28.7, 27.2, 25.7, 25.4, 25.8, 27.3,
        29.0, 32.0, 38.8};
...
// QUARTZ AND TUNGSTEN PLACEMENT ===================================================================================================================================================
         ...
         //= new G4PVPlacement(rotTung,G4ThreeVector(-10.0*mm/4,0,quartz_zPos-quartz_z-8*mm/2),tungsten_box_log,"tungsten", det_log,false,0);

         //G4VPhysicalVolume* tungsten_box_phys_2
         //= new G4PVPlacement(rotTung,G4ThreeVector(-10.0*mm/4,0,quartz_zPos-3*quartz_z-3*8*mm/2),tungsten_box_log,"tungsten", det_log,false,0);

         //G4VPhysicalVolume* tungsten_box_phys_3
         //= new G4PVPlacement(rotTung,G4ThreeVector(-10.0*mm/4,0,quartz_zPos-5*quartz_z-5*8*mm/2),tungsten_box_log,"tungsten", det_log,false,0);

         //G4VPhysicalVolume* tungsten_box_phys_4
         //= new G4PVPlacement(rotTung,G4ThreeVector(-10.0*mm/4,0,quartz_zPos-7*quartz_z-7*8*mm/2-0.5*mm),tungsten_box_log,"tungsten", det_log,false,0); // REPLACE END
//===============================
...
// ================================== LAM - Daniel Valmassei ===================
    ...
    if (fDetMode == 5) {
      ...
      //G4VPhysicalVolume* tmirror_phys = new G4PVPlacement(rot_pmt,G4ThreeVector(7.25*cm+lngth+2.*cm,0.,.9*cm),tmirror_log,"TMirror",det_log,false,0);
      //G4VPhysicalVolume* lightguide_phys = new G4PVPlacement(rotlg,G4ThreeVector(0.*cm,0,-0.375*cm+.9*cm),lightguide_log,"lightguide_phys", det_log,false,0);
      G4VPhysicalVolume* pmt_phys = new G4PVPlacement(rot_pmt,G4ThreeVector(19.5*m,12.*cm,-.5*cm - plngth),pmt_log,"PMT",det_log,false,0,true);
      G4VPhysicalVolume* cath_phys = new G4PVPlacement(rot_pmt,G4ThreeVector(19.5*m,12.*cm,-.5*cm-2.*plngth),cath_log,"CATH",det_log,false,0,true);
