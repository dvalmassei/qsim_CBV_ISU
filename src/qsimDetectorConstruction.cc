...
// ================================== LAM - Daniel Valmassei ===================
 ...
 // QUARTZ DIMENSIONS ======================================================================================================================================================
    ...
    = new G4UnionSolid("QuartzHalf", quartz_box, quartz_box2, rm, G4ThreeVector(0.*cm,18.*cm,12.*cm));

    G4LogicalVolume* quartz_half_log
    = new G4LogicalVolume(quartz_half,Quartz,"QuartzHalf",0,0,0);
...
// ================================== LAM - Daniel Valmassei ===================
    ...
    if (fDetMode == 5) {
      ...
      //G4VPhysicalVolume* tmirror_phys = new G4PVPlacement(rot_pmt,G4ThreeVector(7.25*cm+lngth+2.*cm,0.,.9*cm),tmirror_log,"TMirror",det_log,false,0);
      //G4VPhysicalVolume* lightguide_phys = new G4PVPlacement(rotlg,G4ThreeVector(0.*cm,0,-0.375*cm+.9*cm),lightguide_log,"lightguide_phys", det_log,false,0);
      G4VPhysicalVolume* pmt_phys = new G4PVPlacement(rot_pmt,G4ThreeVector(18.*cm,12.*cm,.5*cm),pmt_log,"PMT",det_log,false,0);
      G4VPhysicalVolume* cath_phys = new G4PVPlacement(rot_pmt,G4ThreeVector(18.*cm,12.*cm,.5*cm),cath_log,"CATH",det_log,false,0);
